...
class LangGraphIntegration(Integration):
    ...
    def setup(self) -> None:
        ...
        logger.debug(
            f"[LangGraph] StateGraph.compile exists: {hasattr(StateGraph, 'compile')}"
        )
        ...
        try:
            ...
            for method_name in [
                ...
                if hasattr(Pregel, method_name):
                    logger.debug(
                        f"[LangGraph] Found Pregel.{method_name} - considering for patching"
                    )

        except ImportError as e:
            logger.warning(f"[LangGraph] Could not import Pregel: {e}")
            # Older versions might not have these internals
    ...
    def _wrap_compile(self, original: Callable) -> Callable:
        ...
        def wrapper(graph_self, *args: Any, **kwargs: Any):  # type: ignore
            ...
            try:
                graph_metadata = self._extract_graph_metadata(
                    graph_self, compiled_graph
                )
            ...
            for method_name in ("invoke", "ainvoke", "stream", "astream"):
                if hasattr(compiled_graph, method_name):
                    try:
                        ...
                        wrapper_func = self._build_graph_wrapper(
                        ...
                        if hasattr(method, "__self__"):
                            ...
                            setattr(
                                compiled_graph,
                                method_name,
                                types.MethodType(wrapper_func, compiled_graph),
                            )
                        else:
                            # Just set it
                        ...
                        logger.debug(
                            f"[LangGraph] Successfully patched {method_name} on compiled graph instance"
                        )
                    except Exception as e:
                        logger.error(
                            f"[LangGraph] Failed to patch {method_name} on instance: {e}"
                        )
                ...
                for edge_info in graph_self._edges:
                    if isinstance(edge_info, tuple) and len(edge_info) >= 2:
                        metadata["edges"].append(
                            {"from": edge_info[0], "to": edge_info[1]}
                        )
    ...
    def _wrap_node_execution(self, original: Callable) -> Callable:
        ...
        if is_async:
            ...
            async def async_wrapper(
                pregel_self, node_name: str, *args: Any, **kwargs: Any
            ):  # type: ignore
                ...
                try:
                    ...
                    try:
                        if len(args) > 0:
                            ...
                            span.set_io(
                                input_data=input_state, output_data=str(result)[:500]
                            )
                    except Exception:
                        pass
    ...
    # ------------------------------------------------------------------
    def _build_graph_wrapper(
        self, original: Callable, method_name: str = None
    ) -> Callable:
        ...
        if method_name is None:
            ...
            elif hasattr(original, "__func__") and hasattr(
                original.__func__, "__name__"
            ):
        ...
        if is_async:
            ...
            async def async_wrapper(compiled_self, *args: Any, **kwargs: Any):  # type: ignore
                start_time = time.time()
                ...
                try:
                    ...
                    if method_name == "astream":
                        return self._wrap_async_generator(
                            result, span, start_time, compiled_self
                        )
        ...
        # Sync version
        # ------------------------------------------------------------------
        def sync_wrapper(compiled_self, *args: Any, **kwargs: Any):  # type: ignore
            start_time = time.time()
            span = self._start_span(compiled_self, method_name)
            ...
            try:
                ...
                if method_name == "stream":
                    return self._wrap_sync_generator(
                        result, span, start_time, compiled_self
                    )
    ...
    # Span helpers
    # ------------------------------------------------------------------
    def _start_span(self, compiled_self, method_name: str):
        attributes = {
            "service.name": "langgraph",
            "kind": "graph",
        ...
        if hasattr(compiled_self, "_ze_graph_metadata"):
            ...
            attributes["has_conditionals"] = (
                len(metadata.get("conditional_edges", [])) > 0
            )
    ...
    def _finalise_span(self, span, start_time: float, args, kwargs, output):
        ...
        try:
            ...
            span.set_io(
                ...
                output_data=output_data or "<no output>",
            )
        except Exception:
    ...
    # Generator wrappers
    # ------------------------------------------------------------------
    def _wrap_sync_generator(self, gen, span, start_time: float, compiled_self=None):
            ...
            try:
                for chunk in gen:
                    if first_chunk_time is None:
                        ...
                        span.attributes["time_to_first_chunk"] = round(
    ...
    def _wrap_async_generator(self, agen, span, start_time: float, compiled_self=None):
        first_chunk_time = None
        node_sequence = []
