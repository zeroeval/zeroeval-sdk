...
def save_to_shell_config(token):
    ...
    try:
        ...
        if system == "Windows":
            ...
            return (
                result.returncode == 0,
                "Windows Registry (System Environment Variables)",
            )
        # Unix-like systems
        ...
        if config_file.exists():
            ...
            if "ZEROEVAL_API_KEY" in content:
                ...
                for i, line in enumerate(lines):
                    if (
                        "export ZEROEVAL_API_KEY=" in line
        ...
        console.print(
            f"[warning]Warning: Could not automatically save to shell config: {e}[/warning]"
        )
        return False, None
        ...
        if success:
            ...
            if location:
                if platform.system() == "Windows":
                    ...
                    console.print(
                        "   [info]Note: You may need to restart your terminal for changes to take effect[/info]"
                    )
                else:
                    console.print(f"   Saved to: {location}")
                    console.print(
                        f"   [info]Run 'source {location}' or restart your terminal to use it[/info]"
                    )
            console.print(
            ...
            console.print(
                "💡 [tip]Best practice: Store this in a .env file in your project root[/tip]"
            )
            console.print("   Create a .env file and add:")
            console.print("   ZEROEVAL_API_KEY=...\n")
