...
class DatasetReader(ABC):
    ...
    @abstractmethod
    def pull_by_id(
        self, dataset_id: str, version_number: Optional[int] = None
    ) -> "Dataset":
        """
...
class DatasetBackendReader(DatasetReader):
        ...
        if self._workspace_id is None:
            try:
                response = requests.post(
                    f"{self.base_url}/api-keys/resolve", json={"api_key": self._api_key}
                )
    ...
    def pull_by_id(
        ...
        dataset_info_url = (
            f"{self.base_url}/workspaces/{self._workspace_id}/datasets/{dataset_id}"
        )
        try:
            ...
            raise RuntimeError(f"Failed to fetch dataset info by ID: {e!s}")
        ...
        try:
            ...
            raise RuntimeError(f"Failed to fetch dataset rows: {e!s}")
    ...
    def pull_by_name(
        ...
        info_url = (
            f"{self.base_url}/workspaces/{self._workspace_id}/datasets/{dataset_name}"
        )
        try:
    ...
    def pull_by_workspace_name(
        ...
        meta_url = (
            f"{self.base_url}/workspaces/name/{workspace_name}/datasets/{dataset_name}"
        )
