...
class MLPipeline:
    ...
    def predict(self, data):
        ...
        current_span = tracer.start_span(
            "predict",
            session_id=self.session_id,
            attributes={"batch_size": len(data), "model_type": "neural_network"},
        )
...
class TimedOperation:
    ...
    def __enter__(self):
        self.span = tracer.start_span(
            self.operation_name, session_id=self.session_id, attributes=self.attributes
        )
        return self.span
...
def process_batch_with_pipeline(batch_id, data):
    ...
    try:
        ...
        with TimedOperation(
            "postprocess_results", session_id, {"result_count": len(predictions)}
        ):
            results = {
                "batch_id": batch_id,
