...
if not sample_images_dir.exists():
    ...
    print(
        "Please create this directory and add sample images before running this script."
    )
    sys.exit(1)

# List available sample images
image_files = list(sample_images_dir.glob("*.jpg")) + list(
    sample_images_dir.glob("*.png")
)
audio_files = list(sample_images_dir.glob("*.mp3")) + list(
    sample_images_dir.glob("*.wav")
)
...
for i in range(min(len(medical_data), len(image_files))):
    ...
    medical_dataset.add_image(
        row_index=i, column_name="chest_xray", image_path=str(image_files[i])
    )
...
if len(audio_files) > 0:
    for i in range(min(2, len(audio_files))):
        medical_dataset.add_audio(
            row_index=i, column_name="verbal_notes", audio_path=str(audio_files[i])
        )
