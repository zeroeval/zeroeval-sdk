...
def get_weather_forecast(location: str, unit: str = "celsius") -> str:
    ...
    if "san francisco" in location.lower():
        return json.dumps(
            {"location": "San Francisco", "temperature": "22", "unit": unit}
        )
    ...
    with span(name="agent.interaction", session_id=session_id) as current_span:
        ...
        if tool_calls:
            ...
            for tool_call in tool_calls:
                ...
                print(
                    f"🛠️ Calling tool: {function_name}({', '.join(f'{k}={v}' for k, v in function_args.items())})"
                )
...
def main():
    ...
    print(
        "This will simulate two conversations with an AI agent, creating two sessions."
    )
    ...
    conversation_1_script = [
        ...
        {
            "role": "user",
            "content": "What is the dot product of the vectors [1, 2, 3] and [4, 5, 6]?",
        },
    ]
